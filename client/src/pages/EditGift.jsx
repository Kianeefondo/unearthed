...
const EditGift = () => {
    ...
    const updateGift = async (event) => {
        ...
        const response = await fetch(`${API_BASE_URL}/gifts/${id}`, options)
        if (!response.ok) {
          console.error('Failed to update gift', await response.text())
          return
        }
    ...
    const deleteGift = async (event) => {
        ...
        const response = await fetch(`${API_BASE_URL}/gifts/${id}`, options)
        if (!response.ok) {
          console.error('Failed to delete gift', await response.text())
          return
        }
